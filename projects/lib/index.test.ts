...
describe("strlcpy", () => {
	it("copies a string to a destination buffer", () => {
		...
		const result = symbols.ft_strlcpy(
			ptrDestination,
			ptrSource,
			destination.length
		);
...
describe("strlcat", () => {
	it("concatenates two strings to a destination buffer", () => {
		...
		const result = symbols.ft_strlcat(
			ptrDestination,
			ptrSource2,
			destination.length
		);
...
describe("toupper", () => {
	it("converts lowercase characters to uppercase", () => {
		...
		for (let i = 0; i < lowercaseChars.length; i++) {
			...
			expect(symbols.ft_toupper(char.charCodeAt(0))).toBe(
...
describe("tolower", () => {
	it("converts uppercase characters to lowercase", () => {
		...
		for (let i = 0; i < lowercaseChars.length; i++) {
			...
			expect(symbols.ft_tolower(char.charCodeAt(0))).toBe(
...
describe("strncmp", () => {
	...
	it("returns a positive value if str1 is greater than str2", () => {
		...
		const ptrStr1 = ptr(Buffer.from(`${str1}\0`, "utf8"));
		const ptrStr2 = ptr(Buffer.from(`${str2}\0`, "utf8"));
		expect(symbols.ft_strncmp(ptrStr1, ptrStr2, length)).toBeGreaterThanOrEqual(
			0
		);
	});
});
